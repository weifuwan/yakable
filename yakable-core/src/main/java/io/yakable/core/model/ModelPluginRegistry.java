...
/**
 * Discovers model plugins through Java ServiceLoader and provides stable provider routing.
 */
public final class ModelPluginRegistry {
    ...
    public static ModelPluginRegistry from(Iterable<ModelPlugin> candidates) {
        Objects.requireNonNull(candidates, "candidates");

        Map<String, ModelPlugin> discovered = new TreeMap<>();
        for (ModelPlugin plugin : candidates) {
            Objects.requireNonNull(plugin, "Model plugin must not be null");
            validateDescriptor(plugin);

            String provider = normalizeProvider(plugin.provider());
            ModelPlugin previous = discovered.putIfAbsent(provider, plugin);
            if (previous != null) {
    ...
    public List<ModelPluginDescriptor> descriptors() {
        List<ModelPluginDescriptor> result = new ArrayList<>(plugins.size());
    ...
    private static void validateDescriptor(ModelPlugin plugin) {
        ...
        if (!descriptor.supports(ModelCapability.CHAT)) {
            throw new IllegalStateException(
                    "Model plugin must support CHAT: " + descriptor.provider()
            );
        }
    }
