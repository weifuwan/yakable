...
test('allows request limits and thinking mode to be configured', () => {
  ...
  assert.equal(config.contextWindowTokens, 64_000);
...
test('rejects invalid request configuration', () => {
  ...
  assert.throws(
    () => resolveDeepSeekRequestConfig({ DEEPSEEK_CONTEXT_WINDOW_TOKENS: '0' }),
    /DEEPSEEK_CONTEXT_WINDOW_TOKENS must be a positive integer/,
  );

  assert.throws(
    () => resolveDeepSeekRequestConfig({
