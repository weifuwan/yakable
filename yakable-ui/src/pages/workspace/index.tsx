...
import { startProjectRuntime } from "@/features/project/api/projects";
import type { ProjectListItem } from "@/features/project/model/types";
import {
  AgentDetailsPanel,
  extractAgentRunsFromConversation,
  type AgentRunView,
} from "@/features/agent-run/components/AgentDetailsPanel";
import { Sidebar } from "@/app/layout/Layout";
import { subscribeFrontendAgentProgress } from "@/features/agent-run/model/frontend-agent";
import { projectTitle } from "@/features/project/model/project";
...
export function WorkspaceShell({
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeFrontendAgentProgress(({ item }) => {
      if (item.type !== "progress" || item.state !== "DONE") return;
