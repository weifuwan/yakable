...
describe('turn-navigation', () => {
  ...
  it('selects Current Turn using a fixed top Reading Anchor', () => {
    expect(currentTurnAtReadingAnchor(layout, 0, 1000)).toBe('turn-1');
    expect(currentTurnAtReadingAnchor(layout, 350, 1000)).toBe('turn-2');
    expect(currentTurnAtReadingAnchor(layout, 700, 1000)).toBe('turn-3');
  });

  it('keeps Visible Turns separate from Current Turn', () => {
    expect(visibleTurnIds(layout, 250, 500)).toEqual(['turn-1', 'turn-2', 'turn-3']);
    expect(currentTurnAtReadingAnchor(layout, 250, 500)).toBe('turn-1');
  });

  it('aligns a Turn top to the Reading Anchor and clamps the scroll range', () => {
    expect(targetScrollTop(layout[2], 500, 1400)).toBe(652);
