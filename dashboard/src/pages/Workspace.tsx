...
function sourceLabel(selection: PersistedVisualSelection): string {
...
function persistedSelection(selection: PreviewSelection): PersistedVisualSelection {
  return {
  ...
  return (
    <header className="grid h-12 shrink-0 [grid-template-columns:var(--editor-chat-width)_minmax(0,1fr)] items-center bg-[#f6f6f4] max-[900px]:grid-cols-[1fr_auto]">
      ...
      <div className="flex min-w-0 items-center gap-2 pr-2 max-[900px]:hidden">
        <ViewSwitcher />
        <PreviewAddressBar onRefresh={onRefresh} previewUrl={previewUrl} routes={routes} currentRoute={currentRoute} onRouteChange={onRouteChange} />
        <EditorActions />
      </div>
    </header>
  );
...
function ChatTimeline({ messages, busy }: { messages: ChatMessage[]; busy: boolean }) {
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-3 z-20 flex justify-center">
      <div ref={toolbarRef} className="pointer-events-auto relative">
        <div data-allow-shadow className={`absolute bottom-[46px] right-0 z-30 w-[190px] origin-bottom-right rounded-xl border border-black/[0.10] bg-white p-1 text-[#1B1B1B] shadow-[0_10px_30px_rgba(15,23,42,0.14),0_2px_6px_rgba(15,23,42,0.08)] transition-[opacity,transform] duration-150 ease-out ${optionsOpen ? "pointer-events-auto translate-y-0 scale-100 opacity-100" : "pointer-events-none translate-y-1 scale-[0.97] opacity-0"}`} role="menu" aria-hidden={!optionsOpen}>
...
export function Workspace({ project, onBack }: { project: ActiveProject; onBack: () => void }) {
  const [runtimeUrl, setRuntimeUrl] = useState(project.previewUrl);
  const [routes, setRoutes] = useState<ProjectRoute[]>(project.routes.length ? project.routes : FALLBACK_ROUTES);
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => conversationMessages(project.conversation, project.summary));
  ...
  async function submitEdit(event: FormEvent) {
    ...
    setPrompt("");
    setMessages((current) => [...current, { id: `pending-user-${createdAt}`, role: "user", content: request, createdAt, visualSelections: selections.map(persistedSelection) }]);
    setBusy(true);
    ...
    try {
      ...
      setCurrentRoute((current) => nextRoutes.some((route) => route.path === current) ? current : (nextRoutes[0]?.path ?? "/"));
      if (result.conversation) setMessages(conversationMessages(result.conversation, result.summary));
      else setMessages((current) => [...current, { id: `assistant-${Date.now()}`, role: "assistant", content: result.changedFiles.length ? `${result.summary}\nChanged: ${result.changedFiles.join(", ")}` : result.summary, createdAt: new Date().toISOString() }]);
    } catch (caught) {
      setMessages((current) => [...current, { id: `error-${Date.now()}`, role: "error", content: caught instanceof Error ? caught.message : "Edit failed.", createdAt: new Date().toISOString() }]);
    } finally {
      setBusy(false);
    }
  }
  ...
  function handleResizeKeyDown(event: React.KeyboardEvent<HTMLDivElement>) {
    if (event.key === "ArrowLeft") { event.preventDefault(); setChatWidth((current) => clampChatWidth(current - 1)); }
    else if (event.key === "ArrowRight") { event.preventDefault(); setChatWidth((current) => clampChatWidth(current + 1)); }
    else if (event.key === "Home") { event.preventDefault(); setChatWidth(MIN_CHAT_WIDTH); }
    else if (event.key === "End") { event.preventDefault(); setChatWidth(MAX_CHAT_WIDTH); }
  }

  return (
    <div className="flex h-screen flex-col overflow-hidden bg-[#f6f6f4] font-sans text-[#252522] antialiased" style={{ "--editor-chat-width": `${chatWidth}%` } as React.CSSProperties}>
      <EditorHeader project={project} onBack={onBack} onRefresh={refreshPreview} previewUrl={previewUrl} routes={routes} currentRoute={currentRoute} onRouteChange={setCurrentRoute} />
      <div ref={panelsRef} className="relative grid min-h-0 flex-1 [grid-template-columns:var(--editor-chat-width)_minmax(0,1fr)] max-[900px]:grid-cols-1 max-[900px]:grid-rows-[45%_55%]">
        <section className="flex min-h-0 flex-col overflow-hidden bg-[#f6f6f4]">
          <ChatTimeline messages={messages} busy={busy} />
          <ChatComposer prompt={prompt} busy={busy} onPromptChange={setPrompt} onSubmit={submitEdit} />
        </section>
        <div className="group/resize-handle absolute inset-y-0 z-30 flex w-3 -translate-x-1/2 cursor-col-resize touch-none items-center justify-center outline-none max-[900px]:hidden" style={{ left: `${chatWidth}%` }} role="separator" aria-label="Resize chat and preview panels" aria-orientation="vertical" aria-valuemin={MIN_CHAT_WIDTH} aria-valuemax={MAX_CHAT_WIDTH} aria-valuenow={Math.round(chatWidth)} aria-valuetext={`${Math.round(chatWidth)}% chat, ${Math.round(100 - chatWidth)}% preview`} tabIndex={0} onPointerDown={(event) => { if (event.button !== 0) return; event.preventDefault(); setIsResizing(true); }} onDoubleClick={() => setChatWidth(DEFAULT_CHAT_WIDTH)} onKeyDown={handleResizeKeyDown}>
          <span className={`pointer-events-none absolute inset-y-3 left-1/2 w-[7px] -translate-x-1/2 rounded-full transition-[opacity,filter] duration-200 ${isResizing ? "opacity-100" : "opacity-70 group-hover/resize-handle:opacity-100 group-focus-visible/resize-handle:opacity-100"}`} style={{ backgroundImage: isResizing ? "linear-gradient(to right, rgba(75,115,255,0.16) 0 3px, rgba(47,111,237,0.92) 3px 4px, rgba(75,115,255,0.16) 4px 7px)" : "linear-gradient(to right, rgba(75,115,255,0.08) 0 3px, rgba(70,76,84,0.68) 3px 4px, rgba(75,115,255,0.08) 4px 7px)", WebkitMaskImage: "linear-gradient(to bottom, transparent 0%, rgba(0,0,0,0.15) 5%, rgba(0,0,0,0.5) 11%, black 18%, black 82%, rgba(0,0,0,0.5) 89%, rgba(0,0,0,0.15) 95%, transparent 100%)", maskImage: "linear-gradient(to bottom, transparent 0%, rgba(0,0,0,0.15) 5%, rgba(0,0,0,0.5) 11%, black 18%, black 82%, rgba(0,0,0,0.5) 89%, rgba(0,0,0,0.15) 95%, transparent 100%)", filter: isResizing ? "drop-shadow(0 0 5px rgba(75,115,255,0.32))" : "drop-shadow(0 0 3px rgba(75,115,255,0.14))" }} />
        </div>
        {isResizing ? <div className="absolute inset-0 z-20 cursor-col-resize" aria-hidden="true" /> : null}
        <section className="relative mb-2 mr-2 flex min-h-0 min-w-0 flex-col overflow-hidden rounded-2xl border border-black/[0.08] bg-white shadow-[0_3px_14px_rgba(15,23,42,0.07)] max-[900px]:m-2">
          <iframe className="min-h-0 w-full flex-1 border-0 bg-white" key={previewUrl} title={`${project.title} preview`} src={previewUrl} sandbox="allow-scripts allow-same-origin allow-forms allow-popups" referrerPolicy="no-referrer" />
