...
V1 不为窄屏和移动端设计独立的 Turn Navigator。窄屏下可以隐藏右侧导航，保留现有 Session 滚动与回到底部能力。
...
当前 Session 少于 3 个可导航 Turn 时，不显示 Turn Navigator。

当前 Session 达到 3 个可导航 Turn 后，桌面端应显示 Turn Navigator。
...
预览面板应明确标记当前 Turn 和用户正在 Hover 或聚焦的 Turn。

Turn 数量较多时，预览面板本身应能够浏览全部 Turn，并自动让当前或正在操作的 Turn 保持在可见区域。

点击 Prompt 预览项，与点击对应导航节点具有相同的跳转行为。
...
当前 Turn 表示用户此刻主要正在阅读的那一轮，而不是简单表示“最后一个加载的 Turn”。

同一个 Turn 的 USER Message 和 ASSISTANT Message 在视口中切换时，当前导航节点不能在两个 Message 之间重复跳变。
...
Turn Navigator 的“回到最新位置”和现有“回到底部”入口可以同时存在，但两者到达的最终位置和当前 Turn 状态必须一致。

### 5.8 Drag-to-Scrub

用户可以在 Turn Navigator 上按住并上下拖动，通过导航条快速浏览长 Session。

拖动开始后，导航应持续反映指针当前对应的 Turn，并让消息区域跟随到相应位置。
...
这不意味着进入 Session 时一次性加载全部 Message 正文。Session 原有“默认最近 50 条 Message、按需向上加载更早历史”的规则继续有效。
...
窄屏下可以隐藏 Turn Navigator，不应为了保留右侧导航而挤压消息正文或输入框。
...
当 Session 少于 3 个可导航 Turn 时，桌面端不显示 Turn Navigator；达到 3 个后显示右侧紧凑导航，且不改变消息正文和输入框原有布局。
...
用户 Hover 或键盘进入 Navigator 后能够看到对应 USER Prompt 预览；长 Prompt 正确省略，当前 Turn 和正在操作的 Turn 能够区分。关闭 Preview 后，Session 阅读位置保持不变。

用户点击任意已加载 Turn 的节点或 Prompt 预览后，页面能够定位到对应 USER Message，目标不被顶部区域遮挡，最终当前 Turn 与目标一致。快速连续点击多个目标时，最终停留在最后一次选择的 Turn。

上一轮和下一轮一次只移动一个 Turn；最早 Turn、最新位置和现有回到底部入口行为一致，不出现已经到底仍可继续无效跳转的状态。

用户可以按住 Navigator 上下拖动，从长 Session 的一个区域快速移动到另一个区域；拖动中的 Preview 与正文位置对应，结束后停留在最终 Turn。轻微移动仍按普通点击处理，不产生误拖动。
...
Session 历史超过首屏最近 50 条 Message 时，Navigator 仍能够到达更早 Turn。点击尚未加载正文的历史 Turn 后，系统先加载必要历史再完成定位；加载失败不会破坏当前阅读位置，并允许重试。
