...
function readDecisions(value: unknown): TasteDecision[] {
  if (!Array.isArray(value)) {
    throw new Error('Taste Translator returned an invalid decisions array.');
  }
  ...
  for (const item of value) {
    if (!isRecord(item)) {
      throw new Error('Taste Translator returned an invalid decision item.');
    }
    ...
    decisions.push({
      ...
      sourceKeywords: readStringArray(
        item.sourceKeywords,
        'decision sourceKeywords',
        MAX_SOURCE_KEYWORDS,
      ),
    });
...
export function parseTasteTranslation(raw: string): TasteTranslation {
  ...
  if (!isRecord(parsed)) {
    throw new Error('Taste Translator must return a JSON object.');
  }
  if (parsed.version !== 1) {
    throw new Error('Taste Translator returned an unsupported translation version.');
  }
  ...
  return {
    ...
    unresolvedDecisions: readStringArray(
      parsed.unresolvedDecisions,
      'unresolvedDecisions',
