import { requestSemanticExpansion } from '../model/capabilities.js';
...
const DEFAULT_KINDS = new Set<SemanticDefaultKind>([
  'structure', 'content', 'capability', 'behavior', 'quality',
]);
const DEFAULT_BASES = new Set<SemanticDefaultBasis>([
  'product-pattern', 'page-pattern', 'goal-pattern', 'universal',
]);
...
function readStringArray(value: unknown, field: string, maxItems: number): string[] {
  if (!Array.isArray(value)) throw new Error(`Semantic Expander returned an invalid ${field}.`);
...
function readDefaults(value: unknown): SemanticDefault[] {
  if (!Array.isArray(value)) throw new Error('Semantic Expander returned an invalid defaults array.');
  const defaults: SemanticDefault[] = [];
  const seen = new Set<string>();
  for (const item of value) {
    if (!isRecord(item)) throw new Error('Semantic Expander returned an invalid default item.');
    ...
    if (defaults.length >= MAX_DEFAULTS) break;
  }
  return defaults;
}
...
export function parseSemanticExpansion(raw: string): SemanticExpansion {
  ...
  if (!isRecord(parsed)) throw new Error('Semantic Expander must return a JSON object.');
  if (parsed.version !== 1) throw new Error('Semantic Expander returned an unsupported expansion version.');

  return {
    version: 1,
    defaults: readDefaults(parsed.defaults),
    assumptions: readStringArray(parsed.assumptions, 'assumptions', MAX_ASSUMPTIONS),
    deferredDecisions: readStringArray(parsed.deferredDecisions, 'deferredDecisions', MAX_DEFERRED_DECISIONS),
  };
}
...
export function buildSemanticExpansionRequest(
  ...
  return JSON.stringify({ productRequest, promptIntent }, null, 2);
}
