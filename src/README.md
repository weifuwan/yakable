...
├── editing/              # normalize edit intent, select/search context, edit, check, and repair once
...
- **editing** keeps the edit loop deliberately bounded. Before context selection, Edit Intent Delta normalizes the current human request against the persisted original Design Intent into a small `summary + scope + targetHints + directives + preserve` contract. That delta is advisory structure for the same request, not a replacement task: the human request still wins on conflict. Editing then selects at most 12 relevant paths, may request exactly one literal project-text search, reads only the focused context, applies one user edit, and runs `check_project`. A PASS ends immediately. A FAIL may trigger exactly one repair using a new bounded context prioritized from initially changed files, diagnostic paths, and the original edit context. The repair can modify only those files, then `check_project` runs exactly once more and editing stops whether the result passes or fails. Source-mapped Visual Edit targets still bypass context-model selection and search. Edit Intent normalization has a deterministic fallback so a normalization failure does not block editing.
...
- **model** owns provider-specific transport for intent analysis, generation, Edit Intent normalization, focused editing, context selection, and the single repair request. Capability modules should not know DeepSeek HTTP details.
