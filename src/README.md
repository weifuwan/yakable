...
├── editing/              # normalize, edit, check, critique, and repair through bounded capabilities
...
- **editing** keeps frontend reasoning deliberately bounded. Before context selection, Edit Intent Delta normalizes the current human request against the persisted original Design Intent into a small `summary + scope + targetHints + directives + preserve` contract. That delta is advisory structure for the same request, not a replacement task: the human request still wins on conflict. Editing then selects at most 12 relevant paths, may request exactly one literal project-text search, reads only focused context, applies one user edit, and runs `check_project`. A FAIL may trigger exactly one build repair. After a healthy edit, the browser-side feedback path can request one Page Observation and send it to Design Critic. A critic PASS ends immediately. A critic FAIL may trigger exactly one Visual Repair using only source files derived from critic evidence, the just-changed files, and the original selected context. Visual Repair cannot create files, cannot chase unverified areas, and runs the fixed project health check exactly once. If that health check fails, Yakable restores the pre-visual-repair source snapshot. If it passes, the Preview is observed once more and critiqued once more, then the loop stops whether the final critique passes or fails.
...
- **server** wires Project Edit, Design Critic, and one Visual Repair attempt into local API boundaries. The browser still owns live Preview observation because the DOM exists inside the Preview frame.
