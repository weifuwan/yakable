import { requestEditIntentDelta } from '../model/capabilities.js';
...
const SCOPES = new Set<EditIntentScope>(['selection', 'component', 'section', 'page', 'project']);
...
function readRequiredString(value: unknown, field: string, maxLength: number): string {
  if (typeof value !== 'string') throw new Error(`Edit Intent Delta returned an invalid ${field}.`);
...
function readDirectives(value: unknown): EditIntentDirective[] {
  if (!Array.isArray(value) || value.length === 0 || value.length > MAX_EDIT_INTENT_DIRECTIVES) {
    throw new Error(`Edit Intent Delta directives must contain 1-${MAX_EDIT_INTENT_DIRECTIVES} items.`);
  }
  const directives: EditIntentDirective[] = [];
  const seen = new Set<string>();
  for (const item of value) {
    if (!isRecord(item)) throw new Error('Edit Intent Delta returned an invalid directive.');
    const area = readEnum(item.area, 'directive area', AREAS);
    const directive = readRequiredString(item.directive, 'directive text', MAX_EDIT_INTENT_DIRECTIVE_LENGTH);
  ...
  if (directives.length === 0) throw new Error('Edit Intent Delta must contain at least one unique directive.');
  return directives;
}
...
export function parseEditIntentDelta(rawContent: string): EditIntentDelta {
  ...
  return {
    ...
    targetHints: readStringArray(value.targetHints, 'targetHints', MAX_EDIT_INTENT_TARGET_HINTS, MAX_EDIT_INTENT_TARGET_HINT_LENGTH),
    directives: readDirectives(value.directives),
    preserve: readStringArray(value.preserve, 'preserve', MAX_EDIT_INTENT_PRESERVE, MAX_EDIT_INTENT_PRESERVE_LENGTH),
  };
}

export function buildEditIntentDeltaRequest(input: EditIntentDeltaInput): string {
  const userRequest = input.userRequest.trim();
  if (!userRequest) throw new Error('Edit Intent Delta requires a follow-up edit request.');
  if (userRequest.length > MAX_EDIT_INTENT_PROMPT_LENGTH) {
    throw new Error(`Edit Intent Delta accepts at most ${MAX_EDIT_INTENT_PROMPT_LENGTH} characters.`);
  }
  ...
  return JSON.stringify({ userRequest, baselineDesignIntent: input.baselineDesignIntent, visualSelections });
}
...
function fallbackScope(request: string, visualSelections: ProjectVisualSelection[]): EditIntentScope {
  ...
  if (/(全局|整个项目|全站|所有页面|global|project-wide|site-wide|everywhere)/i.test(request)) return 'project';
  if (/(整个页面|全页|当前页面|whole page|entire page|page-wide)/i.test(request)) return 'page';
  if (/(hero|pricing|header|footer|section|区域|区块|页头|页脚|导航栏)/i.test(request)) return 'section';
  return 'component';
}
...
function fallbackArea(request: string): EditIntentArea {
  ...
  if (/(圆角|阴影|边框|卡片|表面|radius|shadow|border|surface|card)/i.test(request)) return 'surface-treatment';
  ...
  if (/(响应式|手机|移动端|平板|responsive|mobile|tablet|breakpoint)/i.test(request)) return 'responsive';
...
export function fallbackEditIntentDelta(
  ...
  if (!request) throw new Error('Edit Intent Delta requires a follow-up edit request.');
  ...
  return {
    ...
    directives: [{ area: fallbackArea(request), directive, basis: 'explicit' }],
    preserve: [],
  };
}
...
export async function resolveEditIntentDelta(
  ...
  try {
    const generation = await requestEditIntentDelta(modelClient, request);
    return {
